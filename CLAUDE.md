...
- **Type:** Fraunces (serif, sub-headings/body headings) + Work Sans (body) + Playfair Display SC (hero title + nav wordmark specifically — unchanged) + **IBM Plex Sans Devanagari (2026-08-27, replaced Mukta from a day earlier — Joe didn't like Mukta once he saw it live)** via the `--headline` CSS variable in `style.css`. Applies to: the five main section `<h2>`s (About/Fleet/Book/FAQ/Contact) *and*, as of the same 2026-08-27 follow-up, the Book section's wizard-step `<h3>`s ("Journey details"/"Choose your vehicle"/"Passenger details"/"Review & send" — Joe pointed at these in a screenshot and wanted them styled the same way, broadening the original "just the section headlines" scope). Fleet/vehicle card names and contact item labels (also `h3`) were *not* included — still Fraunces.
  - **Centred, not just re-fonted (fixed 2026-08-27):** the first pass only changed the typeface and missed that Joe also wanted the text centred — `.eyebrow` (was `inline-block`, now `display:block` so its own `text-align` can centre it) and `.section-intro` (added `margin-inline:auto` + `text-align:center`) are centred everywhere they appear, alongside the headline `<h2>`s/wizard `<h3>`s. Deliberately *not* centred: About's two-column body paragraphs (`.about-cols`) and the footer/nav-logo — centring flowing paragraph text or the nav wordmark isn't what "the headlines" meant, so left alone on purpose.
