...
- [ ] **Booking flow — agreed integration plan, not yet built into `index.html`** — Joe confirmed 2026-08-26: it becomes a new section on the homepage (further down, likely right after Fleet — "you've just seen the cars, here's book one"), not a separate page, matching the site's existing single-page/anchor-nav architecture. Add a new **"Book Now"** nav item pointing at it. **Leave "Contact" nav link pointing at the existing simple contact form** — deliberately not repointing it, so casual visitors who just want to ask a quick question aren't funnelled into the 4-step flow. Not yet implemented — still needs: renaming `booking-example.html` → a production name (e.g. `book.html` or inlining as a section in `index.html`), dropping the "Prototype, not live" banner, adding the nav link, then a git push (history) + the usual manual BigWetFish upload (see `website-upload-guide.md`) to actually go live.
